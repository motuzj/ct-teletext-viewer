...
class CTTeletextViewer:
    ...
    def search_word(self, word):
        ...
        """
        self.get_teletext_json(URL)
        for page in self.pages:
            teletext_page = self.get_teletext_page(page)
    ...
    def load_teletext_json(self, filename):
        """
    ...
    def get_teletext_json(self, url):
        """
        ...
        if self.args.input:
            ...
            try:
                json_text = self.load_teletext_json(self.args.input)
    ...
    def get_teletext_page(self, page_num, subpage=None):
        """
        Prints teletext page with optional formatting.

        Parameters:
            page_num (int): Number of a page.
            subpage: Optional subpage name.
        """
        self.print_verbose("Parsing teletext content and applying formatting...")
    ...
    def main(self):
        ...
        while True:
            ...
            print(self.get_teletext_page(self.current_page))
            ...
            while True:
                ...
                if user_input.isnumeric():
                    ...
                    self.current_subpage = user_input.capitalize()
                break
...
def main():
    ...
    parser.add_argument('-g', '--search', type=str, metavar="WORD", help="search for a specific word in all pages (case-sensitive)")
    ...
    if args.page:
        viewer.get_teletext_json(URL)
        print(viewer.get_teletext_page(args.page, args.subpage))
        sys.exit()
    elif args.output:
        viewer.get_teletext_json(URL)
        sys.exit()
    ...
    if not sys.stdout.isatty():
        print("Error: stdout is not supported in interactive mode. Use instead arguments --page and --subpage (optional)", file=sys.stderr)
        sys.exit(1)
    viewer.main()
